...
/*Pin Setup*/
int irPin = 13; //digital pin 13 for Infra Red
int gyroPinX = 0; //analog pin 0 for x axis on Gyroscope
...
void loop() {
  ...
  if((data == midi_clock) && (play_flag == 1)){
    ...
    drumCounter = (drumCounter+1) % 23; //23 is the magic number 
    brightCounter = (brightCounter+1) % 7;
    bassCounter = (bassCounter+1) % 23;
  }
...
int getSoundLevel(){
    //TODO: Scale better
    int soundLevel = ((analogRead(soundPin)/127.0) * 91) +36;
 ...
 */
int getXTilt(){
   int x = (analogRead(gyroPinX) / 512.0) * 16383;
   return x;
}

int getYTilt(){
   int y = ((analogRead(gyroPinY) / 512.0) * 40) + 30;
   return y;
}
...
void syncDrums(int note) {
  MIDI.sendNoteOn(note,69,drumChannel);
}

void syncBass(int note) {
  MIDI.sendNoteOn(note,69,bassChannel);
}
