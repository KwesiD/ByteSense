...
#define SOFTPWM_TIMER_SET(val)     (TCNT1L = (val))
#define SOFTPWM_TIMER_INIT(ocr) ({\
  TIFR1 = (1 << TOV1);    /* clear interrupt flag */ \
  TCCR1A = 0; \
  TCCR1B = (1 << CS11 | 1 << WGM12);   /* start timer (ck/8 prescalar) */ \
  OCR1AL = (ocr);          /* We want to have at least 30Hz or else it gets choppy */ \
  TIMSK1 = (1 << OCIE1A); /* enable timer1 output compare match interrupt */ \
})
#define SOFTPWM_TIMER_END() ({\
  TCCR1B = 0; \
  TIMSK1 = 0; \
})
